...
std::vector<int> LongInt::_string_convert_to_vector(const std::string &string)
{
    ...
    for (long long string_position = string.size() - 1, result_position = result.size() - 1;
         string_position >= 0;
         string_position = string_position - _base_length, result_position = result_position - 1)
    {
        ...
        else
        {
            result[result_position] =
                std::stoi(string.substr((string_position + 1) - _base_length, _base_length));
        }
    }
    return result;
}
...
LongInt LongInt::_zeroes_leading_remove(LongInt number)
{
    ...
    number._digits.erase(number._digits.begin() + 0,
                         number._digits.begin() + zeroes_leading_border);
    return number;
}
...
{
    return (((long long)number._digits.size() - (long long)1) * (long long)_base_length) +
           (long long)std::to_string(number._digits[0]).size();
}
bool operator==(LongInt number_first, LongInt number_second)
{
    ...
    for (long long numbers_position = 0; numbers_position < number_first._digits.size();
...
bool operator>(LongInt number_first, LongInt number_second)
{
    ...
    return !(std::lexicographical_compare(number_first._digits.begin(), number_first._digits.end(),
                                          number_second._digits.begin(),
                                          number_second._digits.end()));
}
...
LongInt operator+(LongInt number_first, LongInt number_second)
{
    ...
    if (number_first._digits.size() > number_second._digits.size())
    {
        number_second = LongInt::_shift_right(number_second, number_first._digits.size() -
                                                                 number_second._digits.size());
    }
    else
    {
        number_first = LongInt::_shift_right(number_first, number_second._digits.size() -
                                                               number_first._digits.size());
    }
    int sum;
    int in_mind = 0;
    for (long long numbers_position = number_first._digits.size() - 1; numbers_position >= 0;
         numbers_position = numbers_position - 1)
    {
        sum = number_first._digits[numbers_position] + number_second._digits[numbers_position] +
              in_mind;
...
LongInt operator-(LongInt number_first, LongInt number_second)
{
    ...
    number_second = LongInt::_shift_right(number_second, number_first._digits.size() -
                                                             number_second._digits.size());
    int different;
    for (long long numbers_position1 = number_first._digits.size() - 1; numbers_position1 >= 0;
         numbers_position1 = numbers_position1 - 1)
    {
        different =
            number_first._digits[numbers_position1] - number_second._digits[numbers_position1];
        if (different >= 0)
        {
        ...
        else
        {
            ...
            for (long long numbers_position2 = numbers_position1 - 1; true;
                 numbers_position2 = numbers_position2 - 1)
            {
                ...
                {
                    number_first._digits[numbers_position2] =
                        number_first._digits[numbers_position2] - 1;
                    break;
                }
            }
        }
...
LongInt LongInt::_multiply_karatsuba(LongInt number_first, LongInt number_second,
                                     bool iteration_thirst)
{
    if (std::min(number_first._digits.size(), number_second._digits.size()) <=
        _length_maximum_for_default_multiply)
    {
        ...
        for (long long number_first_position = number_first._digits.size() - 1;
             number_first_position >= 0; number_first_position = number_first_position - 1)
        {
            for (long long number_second_position = number_second._digits.size() - 1;
                 number_second_position >= 0; number_second_position = number_second_position - 1)
            {
                composition = (long long)number_first._digits[number_first_position] *
                                  (long long)number_second._digits[number_second_position] +
                              result._digits[number_first_position + number_second_position + 1];
                result._digits[number_first_position + number_second_position + 1] =
                    composition % LongInt::_base;
                result._digits[number_first_position + number_second_position + 1 - 1] =
                    result._digits[number_first_position + number_second_position + 1 - 1] +
                    (composition / LongInt::_base);
            }
        }
    ...
    std::copy(number_first._digits.begin() + 0, number_first._digits.begin() + numbers_part_size,
              number_first_part_left._digits.begin() + 0);
    std::copy(number_second._digits.begin() + 0, number_second._digits.begin() + numbers_part_size,
              number_second_part_left._digits.begin() + 0);
    std::copy(number_first._digits.begin() + numbers_part_size,
              number_first._digits.begin() + numbers_size,
              number_first_part_right._digits.begin() + 0);
    std::copy(number_second._digits.begin() + numbers_part_size,
              number_second._digits.begin() + numbers_size,
              number_second_part_right._digits.begin() + 0);
    ...
    {
        auto thread_first = std::async(LongInt::_multiply_karatsuba, number_first_part_left,
                                       number_second_part_left, false);
        auto thread_second = std::async(LongInt::_multiply_karatsuba, number_first_part_right,
                                        number_second_part_right, false);
        product_third = LongInt::_multiply_karatsuba(
            LongInt::_zeroes_leading_remove(number_first_part_left) +
                LongInt::_zeroes_leading_remove(number_first_part_right),
            LongInt::_zeroes_leading_remove(number_second_part_left) +
                LongInt::_zeroes_leading_remove(number_second_part_right),
            false);
    ...
    else if (iteration_thirst and std::thread::hardware_concurrency() == 2)
    {
        auto thread_first = std::async(LongInt::_multiply_karatsuba, number_first_part_left,
                                       number_second_part_left, false);
        product_second =
            LongInt::_multiply_karatsuba(number_first_part_right, number_second_part_right, false);
        product_third = LongInt::_multiply_karatsuba(
            LongInt::_zeroes_leading_remove(number_first_part_left) +
                LongInt::_zeroes_leading_remove(number_first_part_right),
            LongInt::_zeroes_leading_remove(number_second_part_left) +
    ...
    {
        product_first =
            LongInt::_multiply_karatsuba(number_first_part_left, number_second_part_left, false);
        product_second =
            LongInt::_multiply_karatsuba(number_first_part_right, number_second_part_right, false);
        product_third = LongInt::_multiply_karatsuba(
            LongInt::_zeroes_leading_remove(number_first_part_left) +
    ...
    return LongInt::_shift_left(product_first, numbers_size) +
           LongInt::_shift_left(product_third - (product_first + product_second),
                                numbers_part_size) +
           product_second;
}
...
LongInt operator/(LongInt number_first, LongInt number_second)
{
    ...
    if (number_second._digits.size() == 1)
    {
        ...
        {
            composition =
                (long long)number_first._digits[i] + (long long)in_mind * (long long)LongInt::_base;
    ...
    int right;
    LongInt tmp;
    for (long long number_first_position = 0; number_first_position < number_first._digits.size();
         number_first_position = number_first_position + 1)
    {
...
LongInt operator%(LongInt number_first, LongInt number_second)
{
    if (number_second == 0)
    {
        throw "Fatal error. Division remainder calculation is impossible. Attempt to divide by "
              "zero.";
    }
    if (number_second._digits.size() == 1)
    {
...
LongInt LongInt::pow(LongInt number_first, LongInt number_second)
{
    ...
    {
        throw "Fatal error. Pow calculation is impossible. It is impossible to raise zero to zero "
              "degree.";
    }
    if (number_second < 0)
    {
        throw "Fatal error. Pow calculation is impossible. This class only support whole numbers, "
              "so erection to negative degree is impossible.";
    }
    LongInt result = 1;
...
LongInt LongInt::_factorial_tree(LongInt number_first, const LongInt &number_second)
{
    ...
    return LongInt::_factorial_tree(number_first, tmp) *
           LongInt::_factorial_tree(tmp + 1, number_second);
}
LongInt LongInt::factorial(LongInt number)
{
    if (number < 1)
    {
        throw "Fatal error. Factorial calculation is impossible. Factorial is defined only for "
              "natural numbers.";
    }
...
LongInt LongInt::isqrt(const LongInt &number)
{
    if (!number._sign)
    {
        throw "Fatal error. Isqrt calculation is impossible. Isqrt operation over negative numbers "
              "has no result.";
    }
    if (number == 0)
    {
...
std::vector<std::string> divide_strings(std::string a, std::string b)
{
    LongInt l(a);
    LongInt r(b);
    return {LongInt::to_string(l / r), LongInt::to_string(l % r)};
}
